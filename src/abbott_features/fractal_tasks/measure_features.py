...
def measure_features(
    *,
    ...
    # Get label_zarr_url of reference acquisition
    # where the label image and table is stored if provided
    # otherwise assumes each acquisition has its own label image
    if reference_acquisition is not None:
        well_url = Path(zarr_url).parent
        label_zarr_url = well_url / str(reference_acquisition)
    else:
        label_zarr_url = zarr_url

    # Open the OME-Zarr container
    ome_zarr_container = open_ome_zarr_container(zarr_url)

    # Get OME-Zarr container of reference acquisition
    ome_zarr_container_ref = open_ome_zarr_container(label_zarr_url)
    ...
    logging.info(f"Start feature measurement for {label_name=}")
        ...
        if measure_colocalization_features is not None:
            ...
            for channel_pair in measure_colocalization_features.channel_pair:
                ...
                colocalization_roi_table = get_colocalization_features(
                    label_image=label_img,
                    images=images,
                    channel0=channel_0_lbl,
                    channel1=channel_1_lbl,
                    roi=roi,
                )
    ...
    logging.info(f"Finished feature measurement for {label_name=}")
    ...
    logging.info(f"Finished saving table to {store=}")
